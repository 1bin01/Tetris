...
void DrawNextBlock(int *nextBlock) {
    ...
    Max_ptr = NULL;
    max_ = -1;
    RecNode *parent = malloc(sizeof(RecNode));
    for (int i = 0; i < HEIGHT; i++)
        for (int j = 0; j < WIDTH; j++)
            parent->recField[i][j] = field[i][j];
...
int recommend(RecNode *root) {
    if (root->level == 3) return root->accumulatedScore;

    int cur_node_score = 0;
    int ret = 0;  // 미리 보이는 블럭의 추천 배치까지 고려했을 때 얻을 수 있는 최대 점수
    int lv = root->level;

    for (int rot = 0; rot < 4; rot++) {
        for (int x = -4; x < WIDTH + 4; x++) {
            if (!CheckToMove(root->recField, nextBlock[lv], rot, 0, x)) continue;

            for (int y = blockY; y < HEIGHT; y++) {
                if (!CheckToMove(root->recField, nextBlock[lv], rot, y, x)) {
                    y--;

                    RecNode *tmp = malloc(sizeof(RecNode));
                    for (int i = 0; i < HEIGHT; i++)
                        for (int j = 0; j < WIDTH; j++)
                            tmp->recField[i][j] = root->recField[i][j];
                    // memcpy(tmp->recField, root->recField, sizeof(char) * HEIGHT * WIDTH);
                    tmp->level = lv + 1;
                    tmp->parent = root;

                    tmp->recBlockRotate = rot;
                    tmp->curBlockID = nextBlock[lv];
                    tmp->recBlockX = x;
                    tmp->recBlockY = y;

                    cur_node_score = AddBlockToField(tmp->recField, nextBlock[lv], rot, y, x);
                    cur_node_score += DeleteLine(tmp->recField);
                    tmp->accumulatedScore = cur_node_score + root->accumulatedScore;

                    recommend(tmp);
                    if (max_ < tmp->accumulatedScore || (max_ == tmp->accumulatedScore && Max_ptr->recBlockY < tmp->accumulatedScore)) {
                        Max_ptr = tmp;
                        max_ = tmp->accumulatedScore;
                    }

                    // cur_node_score += recommend(tmp);
                    break;
                }
            }
        }
